...
#parser.add_argument('--prefix', dest='prefix', type=str, help='Outfile prefix.', default='', required=False)
...
#parser.add_argument('--no_plot', dest='no_plot', action='store_true', help='Switch off ploting.', default=False, required=False)
...
def get_pcpu_pmem(pid):
    ...
    pcpu = process.cpu_percent() # % of CPU used
    for child in process.children(recursive=True):
        try:
            pmem += child.memory_percent()
            pcpu += child.cpu_percent()
...
if not args.plot: # then write logfile
    ...
    #if args.prefix:
    #    out_file_name = args.prefix+'.'+out_file_name
    #try:
    #    name = args.name
    #    try:
    #        pids = get_pid_by_name(name)
    #    except:
    #        print (pids, "No process called" , args.name , "found",file=sys.stderr)
    #        exit(1)
    #except :
    ...
    out_file_name = args.plot #.split('.')[0] #otherwise log file is provided by user
